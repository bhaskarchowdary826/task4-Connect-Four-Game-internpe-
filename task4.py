...
def draw_board(board):
    ...
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            ...
            pygame.draw.circle(screen, EMPTY_COLOR, (int(c*SQUARESIZE+SQUARESIZE/2), int(r*SQUARESIZE+SQUARESIZE+SQUARESIZE/2)), RADIUS)
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            pygame.draw.rect(screen, EMPTY_COLOR, (0, 0, WIDTH, SQUARESIZE))
            posx = event.pos[0]
            if turn == 0:
